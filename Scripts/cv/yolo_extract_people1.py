...
Примеры запуска:
    ...
    python extract_people.py video.mp4 --draw  # Включить рамки и инфо на видео
...
def main():
    ...
    parser.add_argument(
        "--draw",
        action="store_true",
        help="Включить отрисовку рамок (bounding boxes), ID, траекторий и инфо на итоговом видео",
    )
    parser.add_argument(
        "--trail",
        type=int,
        default=30,
        help="Длина визуального следа трекинга (кадры; 0 = отключить, работает только с --draw)",
    )
    ...
    # ================================================================
    #  ПРЕДВАРИТЕЛЬНЫЙ РАСЧЁТ ТРАЕКТОРИЙ (только если включена отрисовка)
    # ================================================================
    track_centers: dict[int, dict[int, tuple]] = defaultdict(dict)
    if args.draw and args.trail > 0:
        for fidx, dets in frame_detections.items():
            for det in dets:
                tid = det[0]
    ...
    # ================================================================
    #  ВТОРОЙ ПРОХОД: запись выходного видео
    # ================================================================
    print(f"\n{'═'*55}")
    ...
    for seg_idx, (seg_start, seg_end) in enumerate(segments):
        ...
        for fidx in range(seg_start, seg_end + 1):
            ...
            # Отрисовываем рамки только если указан флаг --draw
            if args.draw:
