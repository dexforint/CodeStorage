...
PROMPT_ANALYZE = (
    "Все фрагменты тебе были выданы. Твоя задача - тщательно понять и проанализировать книгу.\n"
    "Какова центральная идея всей книги?\n"
    "Составь план изучения данной книги, не упустив главные детали. Каждый пункт плана должен представлять собой раздел (включающий один или несколько глав). Подробно опиши каждый раздел в 3–5 предложениях. Укажи главную идею каждого раздела.\n"
    "Количество разделов не должно превышать 7."
)

PROMPT_SUMMARY = (
    "Теперь давай двигаться по плану: для каждого раздела сделай детальный и понятный конспект. Конспект должен быть понятен и его должно быть интересно читать. Учти, что я не являюсь не специалистом по теме книги."
    "по теме книги.\n"
    "Не делай всё и сразу. Для каждого сообщения - отдельный раздел.\n"
    "В данном конспекте ты можешь давать свои комментарии как специалист по теме книги. Твои комментарии должны быть полезны.\n"
    "Если есть упражнения - выпиши их.\n"
    "В конце каждого раздела (сообщения) дай проверяющие вопросы с ответами (если они есть).\n"
)
