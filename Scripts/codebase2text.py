...
def codebase_folder_to_llm_text(
    ...
    *,
    ...
    folder_path:
        Путь к корневой папке кодовой базы, которую нужно “упаковать” в текст.
        Пример: r"C:\\Projects\\myrepo"

    allowed:
        Список gitignore-подобных glob-паттернов (НЕ regex), которые определяют,
        какие файлы/папки включать в структуру и потенциально в вывод содержимого.

        - None (по умолчанию): разрешено всё (кроме того, что скрыто hide_in_tree).
        - Если список задан: включаются ТОЛЬКО пути, совпавшие хотя бы с одним паттерном.

        Важно:
        - паттерны применяются к относительным путям в POSIX-виде: "src/main.py"
        - паттерн без "/" матчит по basename (как в .gitignore), например "*.py"
        - паттерн с "/" матчит по относительному пути, например "src/*.py"
        - поддерживается "**" для произвольной глубины, например "src/**/tests/"
        - паттерн, оканчивающийся "/", считается директорией и матчит её и всё внутри

        Примеры:
            allowed=["*.py", "pyproject.toml", "src/**"]
            allowed=["/src/**", "/README.md"]  # только из корня

    ignore_content:
        Список glob-паттернов для файлов/папок, у которых нужно:
          - ПОКАЗЫВАТЬ их в структуре проекта,
          - но НЕ ВЫВОДИТЬ их содержимое в текст кодовой базы.

        Используется для:
          - больших артефактов (например, "data/", "*.csv", "*.parquet")
          - секретов (например, ".env", "*.pem")
          - тяжёлых lock-файлов, если не нужны (например, "package-lock.json")

        Примеры:
            ignore_content=["data/", "*.png", "*.pdf", ".env"]
            ignore_content=["**/*.min.js"]

        Дополнительно:
            Даже если файл НЕ попал под ignore_content, но он бинарный (png, jpg, pdf и т.п.
            по эвристике), его содержимое всё равно не выводится.

    hide_in_tree:
        Список glob-паттернов для полного скрытия из результата:
          - такие файлы/папки НЕ показываются в структуре,
          - и их содержимое НЕ читается/НЕ добавляется,
          - и (важно) если это директория — в неё даже не заходим при обходе.

        Это аналог “полного исключения” (как .gitignore для экспорта).
        Используется для:
          - .git/, node_modules/, build/ и т.п. (обычно мусор для LLM)

        Примеры:
            hide_in_tree=[".cache/", "tmp/", "logs/"]
            hide_in_tree=["/secrets/"]  # только в корне

    use_default_hide_in_tree:
        Если True (по умолчанию), к hide_in_tree автоматически добавляется дефолтный
        набор исключений:
            .git/, .venv/, venv/, node_modules/, dist/, build/, __pycache__/ и т.п.

        Если False — используются ТОЛЬКО ваши hide_in_tree (или ничего, если None).

    out_dir:
        Куда сохранять выходные файлы:
          - None (по умолчанию): текущая рабочая директория (Path.cwd())
          - иначе: указанный путь

        Выходные файлы:
          - если max_output_chars=None: "<rootname>.txt"
          - если max_output_chars задан: "<rootname>__part001.txt", "__part002.txt", ...
          - список включённых файлов: "<rootname>__included_files.txt"

    max_file_chars:
        Ограничение на количество символов содержимого ОДНОГО файла при выводе.
        Нужен как “страховка” от огромных файлов (логи, дампы), даже если они текстовые.

        - None (по умолчанию): не обрезать
        - число: если файл длиннее — берётся начало и добавляется маркер "<TRUNCATED>"

        Пример:
            max_file_chars=50_000

    max_output_chars:
        Ограничение на максимальное количество символов в ОДНОМ выходном текстовом файле
        кодовой базы (part-файле).

        - None (по умолчанию): всё пишется в один файл "<rootname>.txt"
        - число: создаются part-файлы, чтобы каждый был примерно <= max_output_chars

        Главное правило разбиения:
          - “секция” одного исходного файла (его заголовок + fenced-блок + текст)
            НЕ делится между part-файлами.
          - если секция одного файла сама превышает max_output_chars,
            она записывается в отдельный part-файл целиком (то есть этот part может
            получиться > max_output_chars — это намеренно, чтобы не ломать цельность кода).
